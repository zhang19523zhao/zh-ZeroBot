...
import (
	...
	"github.com/zhang19523zhao/zh-ZeroBot/utils/helper"
)
...
func preprocessMessageEvent(e *Event) {
	...
	case e.DetailType == "guild" && e.SubType == "channel":
		log.Infof("["+
			"[bot] 收到频道(%v)(%v-%v)消息 %v : %v", e.GroupID, e.GuildID, e.ChannelID, e.Sender.String(), e.Message)
		processAt()
	default:
		msg := e.RawMessage
		e.IsToMe = true // 私聊也判断为at
		log.Infof("[bot] 收到私聊消息 %v : %v", e.Sender.String(), msg)
		//if msg == "zh" {
